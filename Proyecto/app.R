        ...
        sidebarLayout(
            ...
            sidebarPanel(
                ...
                sliderInput("beta",
                            ...
                            value = 0.4247,
                            step=0.0001),
                #Se obtiene a partir de una regresion de potencias en la diferencia de aumentos por dia hecha en MS Excel. 
                sliderInput("pobTotal",
                            "Variacion de Poblacion Total:",
                            min = 0,
                            max = 12000000,
                            value = 4360000, #Se inicia con poblacion inicial de Liberia, anio 2014. Este y otros datos poblacionales tomados del banco mundial.
                            step=1),
    ...
    server <- function(input, output) {
        ...
        output$errorRK4 <- renderPlot({
            ...
            x <- seq(0,60)
            plot(sirRK4$time, error1, col="red", type="l", xlab="Tiempo (en dias)", ylab="Error porcentual", main="Error porcentual usando RK4")
        })
        ...
        output$errorEuler <- renderPlot({
            ...
            x <- seq(0,60)
            plot(sirEuler$time, error1, col="blue", type="l",xlab="Tiempo (en dias)", ylab="Error porcentual", main="Error porcentual usando Euler")
        })
        ...
        output$siRk4 <- renderPlot({
            ...
            ## Plot
            matplot(x = times, y = out, type = "l",
                    xlab = "Tiempo (Dias)", ylab="Poblacion", main = "SI con Runge-Kutta",
                    lwd = 1, lty = 1, bty = "l", col = 2:4)
